...
<head>
    <title>Code coverage report for node-coap/lib/agent.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">node-coap/lib</a> agent.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">89.46% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>263/294</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>136/172</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>262/293</span>
            </div>
...
<a name='L537'></a><a href='#L537'>537</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
&nbsp;
  this._sock.on('error', <span class="fstat-no" title="function not covered" >fu</span>nction(err) {
    // we are skipping DNS errors
<span class="cstat-no" title="statement not covered" >    if(!hasDNSbug || err.code !== 'ENOTFOUND')</span>
...
&nbsp;
  if (!this._opts.socket)
    this._closing = true
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (this._msgInFlight !== 0)
...
Agent.prototype._handle = function handle(msg, rsinfo, outSocket) {
  var packet = parse(msg)
    ...
    , ackSent = function(err) {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (err &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >req)</span>
<span class="cstat-no" title="statement not covered" >          req.emit('error', err)</span>
&nbsp;
        that._msgInFlight--
        <span class="missing-if-branch" title="if path not taken" >I</span>if (that._closing &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >that._msgInFlight === 0)</span> {
<span class="cstat-no" title="statement not covered" >          that._doClose()</span>
        }
      }
  if (!req) {
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (packet.code != '0.00' &amp;&amp; (req._packet.token.length != packet.token.length || pf.compareBuffers(req._packet.token, packet.token) != 0)) {
...
<span class="cstat-no" title="statement not covered" >    block1 = parseBlockOption(block1Buff)</span>
    // check for error
<span class="cstat-no" title="statement not covered" >    if (!block1) {</span>
<span class="cstat-no" title="statement not covered" >      req.sender.reset()</span>
<span class="cstat-no" title="statement not covered" >      return req.emit('error', new Error('Failed to parse block1'))</span>
    }
  }
&nbsp;
  req.sender.reset()
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if(block1 &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >packet.ack)</span> {
    ...
    var segmentedSender = <span class="cstat-no" title="statement not covered" >req.segmentedSender</span>
<span class="cstat-no" title="statement not covered" >    if(segmentedSender) {</span>
&nbsp;
      //If there's more to send/receive, then carry on!
<span class="cstat-no" title="statement not covered" >      if(segmentedSender.remaining() &gt; 0) {</span>
<span class="cstat-no" title="statement not covered" >        if(segmentedSender.isCorrectACK(packet, block1)) {</span>
<span class="cstat-no" title="statement not covered" >          delete this._msgIdToReq[req._packet.messageId]</span>
<span class="cstat-no" title="statement not covered" >          req._packet.messageId = that._nextMessageId()</span>
<span class="cstat-no" title="statement not covered" >          this._msgIdToReq[req._packet.messageId] = req</span>
<span class="cstat-no" title="statement not covered" >          segmentedSender.receiveACK(packet, block1)</span>
        } else {
<span class="cstat-no" title="statement not covered" >          segmentedSender.resendPreviousPacket()</span>
        }
<span class="cstat-no" title="statement not covered" >        return</span>
      } else {
        // console.log("Packet received done");
<span class="cstat-no" title="statement not covered" >        removeOption(req._packet.options, "Block1")</span>
<span class="cstat-no" title="statement not covered" >        delete req.segmentedSender</span>
      }
    }
  }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      req.sender.reset()</span>
<span class="cstat-no" title="statement not covered" >      return req.emit('error', new Error('failed to parse block2'))</span>
    }
  }
  if (block2) {
&nbsp;
...
&nbsp;
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!block2Val) {
<span class="cstat-no" title="statement not covered" >        req.sender.reset()</span>
<span class="cstat-no" title="statement not covered" >        return req.emit('error', new Error('failed to create block2'))</span>
      }
...
&nbsp;
      ...
      deregister_req.on('error', <span class="fstat-no" title="function not covered" >fu</span>nction() {})
      deregister_req.end()
    })
  } else {
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (++this._lastToken === maxToken)
<span class="cstat-no" title="statement not covered" >    this._lastToken = 0</span>
&nbsp;
...
Agent.prototype.request = function request(url) {
  ...
  var req
    ...
    , option
    , that = this
    , multicastTimeout = url.multicastTimeout !== undefined ? <span class="branch-0 cbranch-no" title="branch not covered" >parseInt(url.multicastTimeout) </span>: 20000
&nbsp;
...
&nbsp;
    var token
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!(packet.ack || packet.reset)) {
...
<span class="cstat-no" title="statement not covered" >      req.segmentedSender = new SegmentedTransmission(block1Buff[0], req, packet)</span>
<span class="cstat-no" title="statement not covered" >      req.segmentedSender.sendNext()</span>
    } else {
      try {
        buf = generate(packet)
      } catch(err) {
...
&nbsp;
  ...
  if (url.multicast === true) {
    req.multicastTimer = setTimeout(function() {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (req._packet.token) {
      ...
      that._msgInFlight--
      <span class="missing-if-branch" title="if path not taken" >I</span>if (that._msgInFlight === 0 &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >that._closing)</span> {
<span class="cstat-no" title="statement not covered" >        that._doClose()</span>
      }
    }, multicastTimeout)
  }
