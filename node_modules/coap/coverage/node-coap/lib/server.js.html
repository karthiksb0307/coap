...
<head>
    <title>Code coverage report for node-coap/lib/server.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">node-coap/lib</a> server.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">86.13% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>267/310</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>148/174</span>
            </div>
...
<span class="cline-any cline-yes">2139x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">310x</span>
...
<span class="cline-any cline-yes">415x</span>
<span class="cline-any cline-yes">415x</span>
<span class="cline-any cline-yes">623804x</span>
<span class="cline-any cline-yes">623804x</span>
<span class="cline-any cline-yes">623804x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">415x</span>
...
&nbsp;
  ...
  this._multicastInterface = options.multicastInterface ? <span class="branch-0 cbranch-no" title="branch not covered" >options.multicastInterface </span>: null
&nbsp;
  ...
  let max = 32768 * 1024
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof options.cacheSize === 'number' &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >options.cacheSize &gt;= 0)</span> {
<span class="cstat-no" title="statement not covered" >    max = options.cacheSize</span>
  }
&nbsp;
  this._lru = new LRU({
      max
    ...
    , dispose:  function(key, value) {
                  <span class="missing-if-branch" title="else path not taken" >E</span>if (value.sender)
                    value.sender.reset()
                }
  })
...
function allAddresses(type) {
    var family = 'IPv4'
    <span class="missing-if-branch" title="if path not taken" >I</span>if (type === 'udp6') {
<span class="cstat-no" title="statement not covered" >        family = 'IPv6'</span>
    }
    var addresses = []
    var interfaces = os.networkInterfaces()
    for (var ifname in interfaces)  {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (interfaces.hasOwnProperty(ifname)) {
...
<span class="cstat-no" title="statement not covered" >      throw new Error('Already listening')</span>
&nbsp;
    return this
  }
&nbsp;
...
&nbsp;
          <span class="missing-if-branch" title="if path not taken" >I</span>if (that._multicastInterface) {
<span class="cstat-no" title="statement not covered" >            that._sock.addMembership(that._multicastAddress, that._multicastInterface)</span>
          } else {
...
&nbsp;
  this._sock.on('error', <span class="fstat-no" title="function not covered" >fu</span>nction(error) {
<span class="cstat-no" title="statement not covered" >    that.emit('error', error)</span>
  })
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (parameters.pruneTimerPeriod) {
    // Start LRU pruning timer
    this._lru.pruneTimer = setInterval(function () {
      that._lru.prune()
    }, parameters.pruneTimerPeriod*1000)
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this._lru.pruneTimer.unref) {
...
<span class="cstat-no" title="statement not covered" >        return response.emit('error', err)</span>
      }
...
&nbsp;
    if (requestedBlockOption.num &lt; 1) {
      <span class="missing-if-branch" title="if path not taken" >I</span>if(this._block2Cache.remove(cache_key)) {
<span class="cstat-no" title="statement not covered" >        debug('first block2 request, removed old entry from cache')</span>
      }
    } else {
  ...
  // else goes here?
  {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if(block1Buff) {
      var blockState = <span class="cstat-no" title="statement not covered" >parseBlockOption(block1Buff)</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      if(blockState) {</span>
        /** @type {{[k:string]:Buffer}} */
        let cachedData = <span class="cstat-no" title="statement not covered" >this._block1Cache.getWithDefaultInsert(cache_key)</span>
        var blockByteSize = <span class="cstat-no" title="statement not covered" >Math.pow(2, 4 + blockState.blockSize)</span>
        var incomingByteIndex = <span class="cstat-no" title="statement not covered" >blockState.sequenceNumber * blockByteSize</span>
        // Store in the cache object, use the byte index as the key
<span class="cstat-no" title="statement not covered" >        cachedData[incomingByteIndex] = request.payload</span>
  
<span class="cstat-no" title="statement not covered" >        if(!blockState.moreBlocks) {</span>
          // Last block
          var byteOffsets = <span class="cstat-no" title="statement not covered" >Object.keys(cachedData)</span>
            .map(<span class="fstat-no" title="function not covered" >(s</span>tr) =&gt; {
<span class="cstat-no" title="statement not covered" >              return parseInt(str)</span>
...
<span class="cstat-no" title="statement not covered" >              return a - b</span>
          ...
          var byteTotalSum = <span class="cstat-no" title="statement not covered" >incomingByteIndex + request.payload.length</span>
          var next = <span class="cstat-no" title="statement not covered" >0</span>
          var concat = <span class="cstat-no" title="statement not covered" >Buffer.alloc(byteTotalSum)</span>
<span class="cstat-no" title="statement not covered" >          for(var i = <span class="cstat-no" title="statement not covered" >0;</span> i &lt; byteOffsets.length; i++) {</span>
<span class="cstat-no" title="statement not covered" >            if(byteOffsets[i] == next) {</span>
              var buff = <span class="cstat-no" title="statement not covered" >cachedData[byteOffsets[i]]</span>
<span class="cstat-no" title="statement not covered" >              buff.copy(concat, next, 0, buff.length)</span>
<span class="cstat-no" title="statement not covered" >              next += buff.length</span>
            } else {
<span class="cstat-no" title="statement not covered" >              throw new Error("Byte offset not the next in line...")</span>
            }
          }
          
<span class="cstat-no" title="statement not covered" >          this._block1Cache.remove(cache_key)</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >          if(next == concat.length) {</span>
<span class="cstat-no" title="statement not covered" >            request.payload = concat</span>
          } else {
<span class="cstat-no" title="statement not covered" >            throw new Error("Last byte index is not equal to the concat buffer length!")</span>
          }
        } else {
          // More blocks to come. ACK this block
<span class="cstat-no" title="statement not covered" >          response.code = "2.31"</span>
<span class="cstat-no" title="statement not covered" >          response.setOption("Block1", block1Buff)</span>
<span class="cstat-no" title="statement not covered" >          response.end()</span>
<span class="cstat-no" title="statement not covered" >          return</span>
        }
      } else {
<span class="cstat-no" title="statement not covered" >        throw new Error("Invalid block state" + blockState)</span>
      }
    }
  }
&nbsp;
...
<span class="missing-if-branch" title="if path not taken" >I</span>if (maxBlock2 &gt; Math.pow(2, (6+4)))
<span class="cstat-no" title="statement not covered" >  maxBlock2 = Math.pow(2, (6+4))</span>
&nbsp;
/**
...
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (requestedBlockOption.size &gt; maxBlock2)
<span class="cstat-no" title="statement not covered" >    requestedBlockOption.size = maxBlock2</span>
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!block2) {
    // this catch never be match,
    // since we're gentleman, just handle it
<span class="cstat-no" title="statement not covered" >    that.statusCode = '4.02'</span>
<span class="cstat-no" title="statement not covered" >    return OutgoingMessage.prototype.end.call(that)</span>
  }
  this.setOption('Block2', block2)
  this.setOption('ETag', _toETag(payload))
&nbsp;
  // cache it
  <span class="missing-if-branch" title="else path not taken" >E</span>if (this._request.token &amp;&amp; this._request.token.length &gt; 0) {
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Tue Aug 10 2021 10:18:42 GMT+0200 (Central European Summer Time)
            </div>
        </div>
